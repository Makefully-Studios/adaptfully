 ...
 * @param {{ pkg: object, deployFolder: string, server?: string, mode?: string, deployment?: string, artifactPath?: string, log?: (message: string) => void, outputRoot?: string, accessToken?: string, encrypt?: boolean, encryptPublicKey?: string, wrapfullyConfig?: object, extractRoot?: string }} options
 */
export async function runAdaptfullyStage(stage, platformKey, options) {
    ...
    const extractRoot = options.extractRoot
        ?? resolveExtractRoot(outputRoot, platformKey);
 ...
 * Each platform extracts to `output/<platformKey>/`.
 *
 * @param {AdaptfullyStage} stage
...
export async function runAdaptfullyStages(stage, platformKeys, options) {
    ...
    for (const platformKey of platformKeys) {
        ...
        try {
            const result = await runAdaptfullyStage(stage, platformKey, options);
...
export async function adaptfullyFromCli(argv = process.argv) {
    ...
    if (!stage || !platformArg) {
        throw new Error(
            ...
            + '  Extracts always go to output/<platform>/ so siblings are preserved.\n'
            + '\n'
            + '  --deployment <key>  Target a single named deployment.\n'
            + '  --artifact <path>   Prior build artifact directory (deploy only; default: ./output/<platform>/ after build).\n'
